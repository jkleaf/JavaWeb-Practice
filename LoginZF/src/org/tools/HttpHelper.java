...
public class HttpHelper {
	...
	public HttpHelper(String urlString, int timeout) throws IOException {
		this(urlString, timeout, gDefaultEncode);
	}

	public HttpHelper(String urlString, String encode) throws IOException {
	...
	public HttpHelper(String urlString, int timeout, String encode) throws IOException {
		try {
...
//    }
	...
	public byte[] GetHttpGragh() throws IOException {
		try {
			Map<String, String> empty = new HashMap<>();
			HttpURLConnection connection = (HttpURLConnection) mUrl.openConnection();
			connection.setConnectTimeout(gDefaultConnectTimeout);
	...
	public HttpResponse Post(Map<String, String> header, Map<String, String> data, boolean isEncode)
			throws IOException {
		try {
            HttpURLConnection connection = (HttpURLConnection) mUrl.openConnection();
            connection.setConnectTimeout(gDefaultConnectTimeout);
            ...
            for (Map.Entry<String, String> object: data.entrySet()) {
                if (isEncode) {
                    buffer.append(URLEncoder.encode(object.getKey(), mEncode))
                            .append("=")
                            .append(URLEncoder.encode(object.getValue() + "", mEncode))
                            .append("&");
                } else {
                    buffer.append(object.getKey())
                            .append("=")
	...
	protected byte[] GetGraghResponse(HttpURLConnection connection) throws IOException {
		try {
...
//    protected String GetAESResponse(HttpURLConnection connection, String key) throws IOException {//decode
...
//    }
	...
	protected HttpResponse GetResponse(HttpURLConnection connection, String encode) throws IOException {
		try {
			int code = connection.getResponseCode();
			String answer = "";
			if (code == 200) {
	...
	protected void InitConnectionInf(HttpURLConnection connection, Map<String, String> header, int timeout) {
